...
import { GetActiveVehicleList, GetActiveVehicleRoute, InsertUpdateDayWiseVehicleRoutes } from '../../Services/appServices/VehicleManagementService';
...
const AssignRouteScreen = () => {
  ...
  const handleProceed = () => {
    ...
    // return
    if (isValidated) {
      dispatch(InsertUpdateDayWiseVehicleRoutes(data, (res) => {
        // "CreatedId": 5,
                ...
                <ScrollView>
                  {
                    ...
                    NewRouteList.map(e => (
                      <Pressable style={styles.selectCard} onPress={() => handleSelectRoute(e.RId, e.RouteName, e.Charge)} key={e.RId}>
